		...
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item label="套餐类型">
									<el-input v-model="query.package_type"></el-input>
								</el-form-item>
				</el-col>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/order_information/table','add') || $check_action('/order_information/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/order_information/table','del') || $check_action('/order_information/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','full_name')" min-width="200">
					...
					<el-table-column prop="phone_number" @sort-change="$sortChange" label="手机号码"
				v-if="user_group == '管理员' || $check_field('get','phone_number')" min-width="200">
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','purchase_time')" min-width="200">
					...
					<el-table-column prop="annotation_notes" @sort-change="$sortChange" label="标注说明"
				v-if="user_group == '管理员' || $check_field('get','annotation_notes')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/order_information/table','set') || $check_action('/order_information/view','set') || $check_action('/order_information/view','get') || $check_action('//table','add') || $check_action('//view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/order_information/table','set') || $check_action('/order_information/view','set') || $check_action('/order_information/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/order_information/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/order_information/get_list?like=0",
				url_del: "~/api/order_information/del?",
		...
		methods: {
			...
			get_user_user_number(id){
				...
				return ret;
															...
															openPayModal(v){
				this.pay_obj.id = v.order_information_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/order_information/set?order_information_id="+this.pay_obj.id);
