					...
					<el-col :span="8">
						<div class="card chart" >
							...
							<div v-if="!list_goods_price.length">订单没有符合条件的数据</div>
																		...
																		<el-col v-if="user_group == '管理员' || $check_figure('/classified_sales/table')" :span="8">
						<div class="card chart">
																														<pieChart v-if="list_classified_sales.length" id="list_classified_sales" :list="list_classified_sales" :title="'分类销售额统计'"></pieChart>
							<div v-if="!list_classified_sales.length">分类销售额没有符合条件的数据</div>
																						</div>
					</el-col>
							<el-col v-if="user_group == '管理员' || $check_figure('/total_sales_by_category/table')" :span="8">
						<div class="card chart">
																														<pieChart v-if="list_total_sales_by_category.length" id="list_total_sales_by_category" :list="list_total_sales_by_category" :title="'分类总销量统计'"></pieChart>
							<div v-if="!list_total_sales_by_category.length">分类总销量没有符合条件的数据</div>
																						</div>
					</el-col>
					</el-row>
...
<script>
	...
	export default {
		...
		created() {
																// 执行分类销售额数据获取
			this.get_list_classified_sales();
					// 执行分类总销量数据获取
			this.get_list_total_sales_by_category();
								...
								(json) => {
									if (json.result) {
										list[i].name = json.result.obj.nickname;
																																											...
																																											// 获取分类销售额统计图数据
			get_list_classified_sales() {
				let data = {};
								let flag = false;
												this.$get("~/api/classified_sales/list_group?groupby=classification_name", data, (json) => {
					if (json.result) {
						var list = json.result.list;
						this.list_classified_sales = list.map((o) => {
							return {
												name: o[1],
												value: o[0]
							};
						...
						if (flag){
							this.get_nickname(this.list_classified_sales,false);
						}
					}
				});
			},
																														// 获取分类总销量统计图数据
			get_list_total_sales_by_category() {
				let data = {};
								let flag = false;
												this.$get("~/api/total_sales_by_category/list_group?groupby=classification_name", data, (json) => {
					if (json.result) {
						var list = json.result.list;
						this.list_total_sales_by_category = list.map((o) => {
							return {
												name: o[1],
												value: o[0]
							};
						...
						if (flag){
							this.get_nickname(this.list_total_sales_by_category,false);
						}
					}
				});
			},
