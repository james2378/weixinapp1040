<template>
	<view class="diy_details diy_div_completed_order">
				...
				<view class="row">
					...
					<view v-if="$check_field('get','completion_time')" class="col-12 col-md-6">
						<view class="diy_title">
							...
							<text>
								{{ $toTime(obj["completion_time"],"yyyy-MM-dd") }}
							</text>
						</view>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/completed_order/get_obj", {
					completed_order_id:this.query.completed_order_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["completed_order_id"] !==0) {
