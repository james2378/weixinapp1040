          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','title') || ($check_field('add','title') || $check_field('set','title'))" label="标题" name="title">
                            <uni-easyinput type="text" v-model="form['title']" v-if="user_group === '管理员' || (form['about_us_id'] && $check_field('set','title')) || (!form['about_us_id'] && $check_field('add','title'))" :disabled="disabledObj['title_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','release_time') || ($check_field('add','release_time') || $check_field('set','release_time'))" label="发布时间" name="release_time">
                    <uni-datetime-picker v-if="user_group === '管理员' || (form['about_us_id'] && $check_field('set','release_time')) || (!form['about_us_id'] && $check_field('add','release_time'))" v-model="form['release_time']" type="date" :disabled="disabledObj['release_time_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','detailed_introduction') || ($check_field('add','detailed_introduction') || $check_field('set','detailed_introduction'))" label="详情介绍" name="detailed_introduction">
                    <uni-easyinput type="textarea" v-model="form['detailed_introduction']" v-if="user_group === '管理员' || (form['about_us_id'] && $check_field('set','detailed_introduction')) || (!form['about_us_id'] && $check_field('add','detailed_introduction'))" :disabled="disabledObj['detailed_introduction_isDisabled']" />
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      form: {
            "title":  '', // 标题
                    "release_time": this.$toTime(new Date().getTime(), "yyyy-MM-dd"),
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/about_us/upload?'),
