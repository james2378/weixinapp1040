<template>
  <view class="page_total_sales_by_category diy_table diy_table_tml" id="total_sales_by_category_table">
    <!-- 筛选模块(开始) -->
    <view>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/total_sales_by_category/view?" v-show="user_group == '管理员' || $check_action('/total_sales_by_category/table','add') || $check_action('/total_sales_by_category/view','add')">添加</navigator>
		...
		<view class="warp">
			<view class="container">
				<view v-for="(o,i) in list" class="row">
										<view v-if="0 && $check_field('get','title')" class="col">
						<view class="view">
                            ...
                            <span>标题</span>
								...
								<text>
									{{ o["title"] }}
													...
													<view v-if="0 && $check_field('get','statistical_time')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>统计时间</span>
                          </view>
								...
								<text>
									{{ o["statistical_time"] }}
													...
													<view v-if="0 && $check_field('get','classification_name')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>分类名称</span>
                          </view>
								...
								<text>
									{{ o["classification_name"] }}
													...
													<view v-if="0 && $check_field('get','total_sales')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>总销量</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/total_sales_by_category/table','del') || $check_action('/total_sales_by_category/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/total_sales_by_category/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/total_sales_by_category/table','set') || $check_action('/total_sales_by_category/view','set') || $check_action('/total_sales_by_category/view','get')" class="btn btn-primary">详情</navigator>
	        					</view>
				</view>
			</view>
		</view>
