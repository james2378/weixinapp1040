<template>
	<view class="page_package_information diy_detail diy_detail--new" id="package_information_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					<view v-if="$check_field('get','package_name')" class="col-12 col-6">
						<view class="view">
					...
					<view v-if="$check_field('get','package_type')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>套餐类型</span>
							</view>
					...
					<view v-if="$check_field('get','package_price')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>套餐价格</span>
							</view>
					...
					<view v-if="$check_field('get','detailed_description')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>详情描述</span>
							</view>
								<view class="diy_field diy_html">
								<rich-text class="rich_text" :nodes="obj['detailed_description']"></rich-text>
							</view>
							</view>
					</view>
				</view>
		...
		<view class="warp">
			<view class="container-fluid container-fluid-comment--new">
				<view class="row">
					<view class="col">
									<button type="button" class="btn btn-primary" v-if="$check_action('/order_information/edit','add')" @click="to_form('/pages/order_information/edit')">
							<span>购买</span>
						</button>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=package_information&source_field=package_information_id&source_id=' + obj['package_information_id']">
							评论
						</navigator>
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "package_information",
					source_field: "package_information_id",
					source_id: obj["package_information_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "package_information",
					source_field: "package_information_id",
					source_id: this.obj["package_information_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/package_information/set?package_information_id=' + _this.obj["package_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.package_name
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/package_information/set?package_information_id=' + obj["package_information_id"], {
					hits
				}, (res) => {
					if(res.result){
