<template>
	<view class="page_package_information diy_list diy_list_page div_list_tml" id="package_information_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
													<uni-search-bar placeholder="搜索套餐名称" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'package_name')">
				...
				<view>
					<view>
						<view class="tab_view">
							<!-- 筛选栏 -->
																		<uni-section title="口味" type="line">
							<uni-data-select
									@change="change_flavor"
									v-model="query['flavor']"
									:localdata="list_flavor"
							></uni-data-select>
							</uni-section>
																				<uni-section title="套餐类型" type="line">
							<uni-data-select
									@change="change_package_type"
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?package_information_id=' + o['package_information_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
		...
		data() {
			return {
				url_get_list: "~/api/package_information/get_list?like=0",
									// 口味选项列表
				list_flavor: [{value:"全部",text:"全部"}],
									// 套餐类型选项列表
				list_package_type: [{value:"全部",text:"全部"}],
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=package_information", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["package_information_id"] === o["source_id"]){
	...
	async get_list_flavor() {
		...
		if(json.result && json.result.list){
			...
			console.error(json.error);
						...
						// 改变分类标签
	change_package_type(val) {
		if (val === "全部" || val === "") {
			this.query.package_type = "";
		}else{
			this.query.package_type = val
		}
		this.search();
	},
