<template>
  <view class="page_package_information diy_table diy_table_tml" id="package_information_table">
    <!-- 筛选模块(开始) -->
    <view>
            ...
            <view class="">
              ...
              <uni-forms :modelValue="query">
                  <uni-forms-item label="套餐名称" name="package_name">
                      ...
                      <uni-forms-item label="口味" name="flavor">
                  <uni-data-select placeholder="请选择"
            v-model="query['flavor']"
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/package_information/view?" v-show="user_group == '管理员' || $check_action('/package_information/table','add') || $check_action('/package_information/view','add')">添加</navigator>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','raw_material')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>原材料</span>
							</view>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','contact_number')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>联系电话</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','detailed_description')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>详情描述</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/package_information/table','del') || $check_action('/package_information/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/package_information/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/package_information/table','set') || $check_action('/package_information/view','set') || $check_action('/package_information/view','get')" class="btn btn-primary">详情</navigator>
					                    																		<!--跨表按钮-->
															<a type="button" size="mini" class="btn btn-primary" v-if="$check_action('/order_information/view','add')" @click="to_form('/pages/order_information/view',o)">
									<span>购买</span>
								</a>
																						</view>
			</view>
	...
	export default {
		...
		data() {
			return {
				...
				list: [],
                                    // 口味列表
                list_flavor: [{value:"",text:"全部"}],
                                    // 套餐类型列表
                list_package_type: [{value:"",text:"全部"}],
                                        			}
		},
		methods: {
          search() {
